...
for marker in \
  ...
  'Lcom/apple/android/music/player/VivoCarLyrics;->onSeek(Ljava/lang/Object;J)V'; do
  count="$(grep -Fc "$marker" "$target" || true)"
  [[ "$count" == 1 ]] || {
    echo "Patched Smali marker must occur exactly once: $marker (found $count)" >&2
    exit 1
  }
done
